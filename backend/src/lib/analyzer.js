...
function getWeekWindow(records) {
  const dates = records.map(r => new Date(r.t));
  const now = new Date();
  // Most recent Monday
...
function validate(records, weekWindow) {
  ...
  const oldReviews = records.filter(r => new Date(r.t) < monday);
  ...
  const cappedAt60 = records.filter(r => r.ms >= 59900);
  ...
  const hasDeckField = records.some(r => r.flds && r.flds.length > 0);
  if (!hasDeckField) {
...
function buildEaseDistribution(records) {
  ...
  records.forEach(r => { counts[r.e] = (counts[r.e] || 0) + 1; });
  return {
    again: counts[1],
    hard: counts[2],
    good: counts[3],
